...
| 6 | **Content widget type** | `ContentWidget/XyzContentWidgetType.php` | Build the form with the route `action` URL; pass `inputAction` JSON with the embed-page URL to the Twig |
...
1. Is the form's content-widget template using `form_start(form, {attr: {
   class: 'cta-form__form form-dialog', 'data-collect': 'true', ...}})`?
2. Is the submit button inside `<div class="widget-actions">` so
   `AbstractWidget._adoptWidgetActions()` moves it into the dialog's action bar?
3. Does the layout's `_widget_content_widget` block override emit
   `data-page-component-module="orofrontend/js/app/components/widget-form-component"`
   with `_wid` + `savedId` + `message` in the options?
4. Is there a hidden `<input name="input_action" value="{…}">` field for the
   non-dialog redirect?

If all four answer "yes", the flow will work end to end.
